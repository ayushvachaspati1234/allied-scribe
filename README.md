...
- Theme: warm sage base, clay accent, ruled-paper hero band, pill buttons (`--ink`, `--accent`, `--bg-hero` in `style.css`).
- Hero visual is a pure HTML/CSS **split before/after**: live session capture card → drafted SOAP note card.
...
- Logo is an inline SVG pulse-line mark in `index.html` (nav + footer) — swap both `<svg class="logo-mark">` blocks to change it.
